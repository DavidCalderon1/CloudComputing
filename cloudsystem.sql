...
(
	...
	ValorUnit_Producto int not null,
	Producto_IdProveedor int not null,
	primary key (IdProducto),
	constraint FK_producto_proveedor foreign key (Producto_IdProveedor) references [CloudBD1].compra.proveedor(IdProveedor)
); 

create table compra.pedido
(
	...
	constraint FK_pedido_producto foreign key (Pedidos_IdProducto) references [CloudBD1].compra.producto(IdProducto)
);
...
CREATE TABLE RRHH.tipo_usuario (
...
INSERT INTO RRHH.tipo_usuario (IdTipo_Usuario, Nom_Tipo_Usuario) VALUES
(1, 'COLABORADOR'),
(2, 'ADMINISTRADOR'),
(3, 'SOPORTE'),
(4, 'AUDITOR');
...
CREATE TABLE RRHH.usuario (
  ...
  constraint FK_usuario_tipo_usuario foreign key (Usuario_IdTipo_Usuario) references [CloudBD1].RRHH.tipo_usuario(IdTipo_Usuario)
);

--
--  Datos para la tabla usuario
--
INSERT INTO RRHH.usuario (IdUsuario, Usuario_Nombre, Usuario_Password, Usuario_IdTipo_Usuario, Rol_Usuario, Estado_Usuario) VALUES
...
(
	...
	constraint FK_factura_usuario foreign key (Factura_IdUsuario) references [CloudBD1].RRHH.usuario(IdUsuario),
	constraint FK_factura_cliente foreign key (Factura_IdCliente) references [CloudBD1].venta.cliente(IdCliente)
);

create table venta.producto_factura
(
	IdProd_Factura int not null primary key,
	...
	constraint FK_prod_factura_producto foreign key (Prod_Factura_IdProducto) references [CloudBD1].compra.producto(IdProducto),
	constraint FK_prod_factura_factura foreign key (Prod_Factura_IdFactura) references [CloudBD1].venta.factura(IdFactura)
);